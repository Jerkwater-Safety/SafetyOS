...
// ─── PRIVATE STORAGE (per-user, session-isolated) ─────────────────────────────
...
function OnboardingWizard({ user, storage, onComplete }) {
  ...
  const handleFileUpload = (e, setter, type) => {
    ...
    reader.onload = (ev) => {
      // Store privately per user — metadata only in state, content keyed to user
      ...
      storage.set(fileKey, {
        ...
        contentPreview: ev.target.result.substring(0, 500), // first 500 chars only
      });
